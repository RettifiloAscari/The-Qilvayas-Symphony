...
**The production pipeline lives in the repository, at `scripts/` and `tools/`** — read through the GitHub connector, the same as the corpus. These are the real assets, not descriptions of them: read `PIPELINE_README.md` first, then copy the pipeline files out of `scripts/` into the working directory before generating anything. The repository is the single source of truth for them; there is no separate project-knowledge copy to keep in sync, so avoid gratuitous reformatting only because clean diffs keep the history reviewable.

**The mechanics are documented once, in the `qs-build` skill** at `.claude/skills/qs-build/SKILL.md` — the escape convention, the table and `columnWidths` rules, the DM-marker convention, the anchored-edit discipline, and the verification loop. A Claude Code session loads it on demand; a Chat session should read that file directly before editing a generator. It is the single copy: do not restate its rules here or in `CLAUDE.md`, because two copies drift and this project has a mirror rule precisely to stop that.
